...
WHAT THIS TOUCHES
    ...
    - removes <config>/skills/brain/ ONLY if its SKILL.md carries the marker
      (leaves <config>/skills/ for other skills)
    - removes the generated launchers: <config>/brain-agent.py (every platform),
      <config>/brain-launch.cmd and the legacy <config>/brain.cmd (Windows) --
      each only when it says a Brain installer generated it
...
# One marker, one predicate, shared with the installer: the installer backs up a
# file without it, this script refuses to delete one without it.
MARKER = brain_settings_merge.MANAGED_MARKER

# Every per-config-dir file the installer generates, past and present. `brain.cmd`
# is the retired model-facing wrapper; installs made before 2026-09-01 still have
# one, and until then no uninstaller removed it.
GENERATED_LAUNCHERS = ("brain-agent.py", "brain-launch.cmd", "brain.cmd")
...
def remove_managed_claude_md(claude_dir: Path) -> None:
    ...
    if not claude_md.exists():
        ...
        return
    if brain_settings_merge.has_managed_marker(claude_md):
        try:
            claude_md.unlink()
...
def remove_brain_skill(claude_dir: Path) -> None:
    ...
    if not brain_settings_merge.has_managed_marker(skill_md):
        info("       [warn] skills/brain/SKILL.md has no managed-by marker — leaving it in place.")
...
def remove_generated_launchers(claude_dir: Path) -> None:
    """Delete every launcher a Brain installer generated here, on any platform.

    Includes the legacy `brain.cmd`: it was the model-facing wrapper until the
    command-injection fix replaced it with `brain-agent.py`, and no uninstaller ever
    removed it. A file at one of these names that does not say it was generated is
    left alone and reported.
    """
    found = False
    for name in GENERATED_LAUNCHERS:
        path = claude_dir / name
        if not path.exists():
            continue
        found = True
        if not brain_settings_merge.is_generated_launcher(path):
            info(f"       [warn] {name} does not say it was generated by a Brain installer — leaving it.")
            continue
        try:
            path.unlink()
            info(f"       ✓ removed {name}")
        except OSError as e:
            info(f"       [warn] could not remove {path}: {e}")
    if not found:
        info("       (none present)")
...
def _venv_still_referenced(uninstalled: list[Path]) -> Path | None:
    ...
    for cand in sorted(Path.home().glob(".claude*")):
        ...
        for probe in (
            cand / "settings.json",
            cand / "CLAUDE.md",
            cand / "skills" / "brain" / "SKILL.md",
            cand / "brain-launch.cmd",
        ):
            if not probe.exists():
                continue
            try:
                text = probe.read_text(encoding="utf-8", errors="replace")
                if any(v in text for v in venv_variants):
                    return cand
            except OSError:
                continue
    return None
...
def uninstall_one(claude_dir: Path) -> None:
    ...
    step(5, total, f"removing generated launchers from {claude_dir}")
    remove_generated_launchers(claude_dir)
