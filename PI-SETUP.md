...
| `BRAIN_AGENT_SURFACE` | forced to `0` | Set by the extension itself. Claude Code's generated `brain-agent.py` launcher sets it to `1`, which makes the CLI refuse `--from-pi`; the extension is the operator here, so it clears it. |
