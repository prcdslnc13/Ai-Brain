...
#     ~/src/Ai-Brain/setup-mac.sh ~/.claude-work      ~/Documents/Vaults/Ai-Brain --with-mcp
#
...
# (for sessions/clients that need the brain_* tools). Without --with-mcp, any
# existing user-scope 'brain' MCP registration is REMOVED so the token saving
# actually lands.
#
...
#     round-trip — that uncapped network call caused a recall hang). Override with
